  ...
  for (unsigned int p = 0; p < contents.size(); p++){
    ...
    for (unsigned int i = 0; i < splitGuts.size(); i++){
      ...
      if (splitGuts[0] != "%"){ // If the line starts with a % then its a comment and shall be ignored
        if (splitGuts[i] == "let" && splitGuts[i+2] == "="){ _bt.call_def_var(splitGuts[i+1], splitGuts[i+3], false); break; } // variable dec
        ...
        else if ((splitGuts[i] == "python") && (splitGuts[i+1] == "{" || splitGuts[i+2] == "{") && !_bt.call_local_check_python_operation()){
          _bt.call_local_python((splitGuts[i+1] != "{")?splitGuts[i+1]:"NULL", gatherGuts(p, contents));
          break;
        }
        else if ((splitGuts[i] == "asm") && (splitGuts[i+1] == "{" || splitGuts[i+2] == "{") && !_bt.call_local_check_asm_operation()){
        ...
        else if (splitGuts[i] == "sql" && splitGuts[i+2] == "{" && !_bt.call_local_check_sql_operation()){
          _bt.call_local_sql(splitGuts[i+1], gatherGuts(p, contents));
          break; }
        else{
          ...
          if ((A || C) && splitGuts[i+1] == "="){
            if (splitGuts.size() == 5){ // Do we have enough for mathematical operations?
              std::string firstValue = splitGuts[2], secondValue = splitGuts[4], operatorValue = splitGuts[3];
              if (A && A->constValue){ error("Can't modify a const, you can only read from it!"); break; }
              else if (A){ A->value = _bt.call_math_start(firstValue, secondValue, operatorValue); }
              else if (C){ _bt.call_array_upd_at_inx(C, _bt.call_get_array_index(splitGuts[i]), _bt.call_math_start(firstValue, secondValue, operatorValue)); break; }
              break;
            }
            else if (A){ _bt.call_upd_var(A, splitGuts[i+2]); break; } // Update the variable
            else if (C){ _bt.call_array_upd_at_inx(C, _bt.call_get_array_index(splitGuts[i]), splitGuts[i+2]); break; } // Update the array
          }
